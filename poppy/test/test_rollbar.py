...
import poppy
from poppy.lib import python_major_version, string_types

from poppy.test import BaseTest
...
_default_settings = copy.deepcopy(poppy.SETTINGS)


class RollbarTest(BaseTest):
    def setUp(self):
        poppy._initialized = False
        poppy.SETTINGS = copy.deepcopy(_default_settings)
        poppy.init(_test_access_token, locals={'enabled': True}, dummy_key='asdf', handler='blocking', timeout=12345)

    def test_merged_settings(self):
        expected = {'enabled': True, 'sizes': poppy.DEFAULT_LOCALS_SIZES, 'safe_repr': True, 'scrub_varargs': True, 'whitelisted_types': []}
        self.assertDictEqual(poppy.SETTINGS['locals'], expected)
        self.assertEqual(poppy.SETTINGS['timeout'], 12345)
        self.assertEqual(poppy.SETTINGS['dummy_key'], 'asdf')

    def test_default_configuration(self):
        self.assertEqual(poppy.SETTINGS['access_token'], _test_access_token)
        self.assertEqual(poppy.SETTINGS['environment'], 'production')

    @mock.patch('poppy.send_payload')
    def test_disabled(self, send_payload):
        poppy.SETTINGS['enabled'] = False

        poppy.report_message('foo')
        try:
            raise Exception('foo')
        except:
            poppy.report_exc_info()

        self.assertEqual(send_payload.called, False)

    def test_server_data(self):
        server_data = poppy._build_server_data()
        ...
        poppy.SETTINGS['branch'] = 'master'
        poppy.SETTINGS['root'] = '/home/test/'
    ...
    def test_wsgi_request_data(self):
        ...
        data = poppy._build_wsgi_request_data(request)
    ...
    def test_exception_filters(self, send_payload):
        ...
        poppy.SETTINGS['exception_level_filters'] = [
            (OSError, 'ignored'),
            ('poppy.ApiException', 'ignored'),
    ...
    def test_report_exc_info_level(self, send_payload):
        ...
        try:
            ...
            poppy.report_exc_info(level='info')
    ...
    def test_send_failsafe(self, send_payload):
        ...
        test_data = {
            ...
            'data': {
                ...
                'environment': poppy.SETTINGS['environment'],
                'internal': True,
                'notifier': poppy.SETTINGS['notifier']
            }
        }

        poppy._send_failsafe('test message', test_uuid, test_host)
        self.assertEqual(send_payload.call_count, 1)
        self.assertEqual(json.loads(send_payload.call_args[0][0]), test_data)

    @mock.patch('poppy.log.exception')
    @mock.patch('poppy.send_payload', side_effect=Exception('Monkey Business!'))
    ...
    def test_dont_scrub_star_args(self, send_payload):
        poppy.SETTINGS['locals']['scrub_varargs'] = False
    ...
    def test_all_project_frames_have_locals(self, send_payload):
        ...
        prev_root = poppy.SETTINGS['root']
        poppy.SETTINGS['root'] = __file__.rstrip('pyc')
        try:
            step1()
        except:
            poppy.report_exc_info()
        finally:
            poppy.SETTINGS['root'] = prev_root
    ...
    @mock.patch('poppy.lib.transport.post', side_effect=lambda *args, **kw: MockResponse({'status': 'OK'}, 200))
    ...
    def test_scrub_webob_request_data(self):
        poppy._initialized = False
        poppy.init(_test_access_token, locals={'enabled': True}, dummy_key='asdf', handler='blocking', timeout=12345,
                   scrub_fields=poppy.SETTINGS['scrub_fields'] + ['token', 'secret', 'cookies', 'authorization'])
        ...
        unscrubbed = poppy._build_webob_request_data(request)
        ...
        scrubbed = poppy._transform(unscrubbed)
