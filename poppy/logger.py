...
class RollbarHandler(logging.Handler):
    ...
    def __init__(self,
        ...
        if access_token is not None:
            poppy.init(access_token, environment)
    ...
    def emit(self, record):
        ...
        # to Rollbar
        if record.name == poppy.__log_name__:
            return
        ...
        # load the request
        request = getattr(record, "request", None) or poppy.get_request()
        ...
        try:
            ...
            if exc_info and exc_info[0]:
                if message:
                    ...
                    payload_data = poppy.dict_merge(payload_data, message_template)

                uuid = poppy.report_exc_info(exc_info,
                                             level=level,
                                             request=request,
                                             extra_data=extra_data,
                                             payload_data=payload_data)
            else:
                uuid = poppy.report_message(message,
                                            level=level,
                                            request=request,
                                            extra_data=extra_data,
                                            payload_data=payload_data)
