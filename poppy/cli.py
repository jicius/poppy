...
def _gen_report_message(level):
    def _wrapped(lines):
        ...
        poppy.report_message(line_data, level=level, extra_data={'cli_version': VERSION})
    return _wrapped
...
def main():
    ...
    parser.add_option('-t', '--access_token',
                      dest='access_token',
                      help="You project's access token from poppy.com.",
    ...
    parser.add_option('-u', '--url',
                      ...
                      default=poppy.DEFAULT_ENDPOINT)
    ...
    poppy.init(access_token, environment=env, endpoint=endpoint, handler=handler)
