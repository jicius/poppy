...
try:
    ...
    from poppy.contrib.django.utils import MiddlewareMixin

log = logging.getLogger(__name__)


DEFAULTS = {
    'web_base': 'https://poppy.com',
    'enabled': True,
...
{% endif %}
    ...
    def new_get_traceback_data(exception_reporter):
        ...
        try:
            item_uuid = exception_reporter.request.META.get('poppy.uuid')
            if item_uuid:
                url = '%s/item/uuid/?uuid=%s' % (rollbar_web_base, item_uuid)
                data['view_in_rollbar_url'] = url
        except:
            log.exception("Exception while adding view-in-poppy link to technical_500_template.")
        return data
...
class RollbarNotifierMiddleware(MiddlewareMixin):
    def __init__(self, get_response=None):
        ...
        def hook(request, data):
            ...
            if request:
                request.META['poppy.uuid'] = data['uuid']
