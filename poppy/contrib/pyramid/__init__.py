...
DEFAULT_WEB_BASE = 'https://poppy.com'
BOOLEAN_SETTINGS = [
    'poppy.enabled', 'poppy.allow_logging_basic_config',
    'poppy.verify_https'
]
...
def handle_error(settings, request):
    poppy.report_exc_info(sys.exc_info(), request)
...
def patch_debugtoolbar(settings):
    """
    Patches the pyramid_debugtoolbar (if installed) to display a link to the related poppy item.
    """
    try:
    ...
    rollbar_web_base = settings.get('poppy.web_base', DEFAULT_WEB_BASE)
    ...
    def insert_rollbar_console(request, html):
        ...
        item_uuid = request.environ.get('poppy.uuid')
        if not item_uuid:
            return html
...
def includeme(config):
    ...
    config.add_tween('poppy.contrib.pyramid.rollbar_tween_factory', under=EXCVIEW)

    # run patch_debugtoolbar, unless they disabled it
    if asbool(settings.get('poppy.patch_debugtoolbar', True)):
    ...
    def hook(request, data):
        ...
        if request:
            request.environ['poppy.uuid'] = data['uuid']
    ...
    poppy.init(access_token, environment, **kw)
