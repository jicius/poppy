...
from poppy.lib import dict_merge, parse_qs, text, transport, urljoin, iteritems
...
VERSION = __version__
DEFAULT_ENDPOINT = 'https://api.poppy.com/api/1/'
DEFAULT_TIMEOUT = 3
...
from poppy.lib.transforms.scrub_redact import REDACT_REF

from poppy.lib import transforms
from poppy.lib.transforms.scrub import ScrubTransform
from poppy.lib.transforms.scruburl import ScrubUrlTransform
from poppy.lib.transforms.scrub_redact import ScrubRedactTransform
from poppy.lib.transforms.serializable import SerializableTransform
from poppy.lib.transforms.shortener import ShortenerTransform
...
def report_exc_info(exc_info=None, request=None, extra_data=None, payload_data=None, level=None, **kw):
    ...
    poppy.init(access_token='YOUR_PROJECT_ACCESS_TOKEN')
    try:
        do_something()
    except:
        poppy.report_exc_info(sys.exc_info(), request, {'foo': 'bar'}, {'level': 'warning'})
    """
    if exc_info is None:
...
def send_payload(payload, access_token):
    ...
    - 'agent': writes to a log file to be processed by poppy-agent
...
def _create_agent_log():
    """
    Creates .poppy log file for use with poppy-agent
    """
    log_file = SETTINGS['agent.log_file']
    if not log_file.endswith('.poppy'):
        log.error("Provided agent log file does not end with .poppy, which it must. "
...
def _check_config():
    if not SETTINGS.get('enabled'):
        log.info("pyrollbar: Not reporting because poppy is disabled.")
        return False
    ...
    if not SETTINGS.get('access_token'):
        log.warning("pyrollbar: No access_token provided. Please configure by calling poppy.init() with your access token.")
        return False
